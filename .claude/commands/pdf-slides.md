...
1. **Identify the file**: The user may provide:
   ...
   - Just a filename — look in `output/slides/`
   - No argument — find the most recent `.pptx` in `output/slides/`
2. **Convert to PDF**: Run the following command via Bash:
   ...
   ```
   Save the PDF alongside the original in `output/slides/`.
...
- If the file is not found, list available `.pptx` files in `output/slides/`
